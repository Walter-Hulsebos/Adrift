...
#undef STACKED_CAMERA

#ifdef REQUIRE_DEPTH
#if defined(STACKED_CAMERA) //Depth is copied at the render point to encompass the entire camera stack
TEXTURE2D_X_FLOAT(_BlendedDepth);
SAMPLER(sampler_BlendedDepth);

float SampleSceneDepth(float2 uv)
{
	return SAMPLE_TEXTURE2D_X(_BlendedDepth, sampler_BlendedDepth, UnityStereoTransformScreenSpaceTex(uv)).r;
}
#else
